...
export default function ConfessionCard({
  ...
  const [hasLiked, setHasLiked] = useState(() =>
    getStoredLikedState(confession.id),
  );
  ...
  const [tiltStyle, setTiltStyle] = useState({
    ...
    boxShadow:
      "0 10px 30px -10px rgba(0, 0, 0, 0.6), 0 0 1px 1px rgba(168, 85, 247, 0.15)",
  });
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      if (response && typeof response.likes === "number") {
        ...
        const actualLiked =
          typeof response.liked === "boolean" ? response.liked : nextHasLiked;
      ...
      console.error(
        `Lỗi khi cập nhật lượt thích cho confession #${confession.id}:`,
        err,
      );
      // Rollback on failure
