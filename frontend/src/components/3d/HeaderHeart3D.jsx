...
export default function HeaderHeart3D() {
  const fallback = (
    <div
      className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-900 to-purple-950 border border-purple-800/50 flex items-center justify-center shadow-md shadow-purple-950/50 transform hover:scale-105 transition-transform"
      aria-label="Heart Icon Fallback"
    >
      <Heart className="w-6 h-6 text-purple-400 fill-purple-400/80" />
    </div>
  );
    ...
    <WebGLBoundary fallback={fallback}>
      ...
      >
        ...
        >
          ...
          <directionalLight position={[2, 3, 4]} intensity={1.6} color="#f5d0fe" />
          <pointLight position={[-2, -1, 1]} color="#c084fc" intensity={2.6} />
          <pointLight position={[2, -2, -1]} color="#7c3aed" intensity={1.6} />
